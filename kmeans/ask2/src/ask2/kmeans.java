...
public class kmeans {
	...
	//main
		public static void main(String[] args) throws Exception {
			...
			// gia na dimiourgisoume to plot me ta sfalmata omadopoihshs 
		...
		//trexei to programma mas gia 20 epanalipseis gia kathe M
		public static float  epanalipsi(Map<Float, Float> mapFromFile1,int M) throws IOException {
			...
			//System.out.println("epanalipsh"+epanalipsh);
		...
		public static void makePlot2( Map<Float, Float> mapFromFile1) throws IOException {
		     ...
		     float s1 = epanalipsi(mapFromFile1,3);
		     float s2 = epanalipsi(mapFromFile1,5);
		     float s6 = epanalipsi(mapFromFile1,7);
		     float s4 = epanalipsi(mapFromFile1,11);
		     float s5 = epanalipsi(mapFromFile1,13);
		     float s3 = epanalipsi(mapFromFile1,9);
		...
		//testaroume an bgazoume ta swsta apotelesmata gia oti mas zitithike 
			...
			while(count<M) {
				for(int w=0;w<150;w++) {
					...
					if(t[w][2]==count  ) {//flags pinakas x y kai flag
						...
						System.out.println("grammi : " + w +"  to flag einai to :" +t[w][2]+"  to kentro:  "+ nea[count][0]+" "+ nea[count][1]+ "  simeio  " + t[w][0]+" "+t[w][1]+"  sfalma  "+sfalma[w] );
		...
		//dimiourgoume ta tuxaia arxika kentra 
		...
		public static float calculateDistanceBetweenPoints(float x1, float y1, float x2, float y2) {       
		  ...
		  // briskoume thn elaxisth timh gia 2D array
		...
		public static float[][] makeflags( int M,float [][] kentra,Map<Float, Float> mapFromFile1 ){
			...
			for (Float r : mapFromFile1.keySet()) {
				...
				for(int i =0;i<M;i++) {
					d[i]=calculatePower(kentra[i][0],kentra[i][1],r,mapFromFile1.get(r));
		...
		//blepoume an ta nea kentra einai isa me ta palia 
		public static float[][] compareCentrers(float [][]palia, int M,float [][] pointWflags,Map<Float, Float> mapFromFile1 ){
			...
			if (Arrays.deepEquals(palia, nea)){
				...
				//System.out.println("neos pinakas"); 
				//pritnArray2D(nea);
		...
		//briskoume to sfalma omadopoisis
		public static float sfalmaOmadopoisis (float [][]kentra,float [][]flags,int M) {
			...
			//System.out.println("array gia sfalmata");
